...
void setlinearMatrix(const Eigen::Matrix<double,2*Nu*Nc,Nu*Nc+1> &A_cons,Eigen::SparseMatrix<double> &linearMatrix)
{
    ...
    for(int i=2*Nu*Nc;i<2*Nc*Nc+Nc+1;i++)
    {
        for(int j = 0; j<Nu*Nc+1;j++)
        {
            linearMatrix.insert(i,j) = eye_6(i,j);
        }
    }
}
...
void mpc_follow::mpc_core()
{
    ...
    Eigen::Matrix<double,Np*Ny,Nc*Nu> THETA;
    ...
    Eigen::Matrix<double,Np*Ny,1> Yita_ref;
    ...
    */
   //对使用OSQPEigen的约束求解矩阵赋值
    // Eigen::SparseMatrix<double,Nu*Nc+1,Nu*Nc+1> hessian;
    // Eigen::Matrix<double,Nu*Nc+1,1> gradient;
    // Eigen::Matrix<double,2*Nc+Nc+1,1> lowerBound;
    // Eigen::Matrix<double,2*Nc+Nc+1,1> upperBound;
    // Eigen::Matrix<double,2*Nu*Nc+Nc+1,Nu*Nc+1> linearMatrix;

    // hessian = H;

    // gradient = f;

    // lowerBound.block<2*Nc,1>(0,0) = Eigen::Matrix<double,2*Nc,1>::Ones() * -OsqpEigen::INFTY;
    // lowerBound.block<Nc+1,1>(2*Nc,0) = lb;
    // upperBound.block<2*Nc,1>(0,0) = b_cons;
    // upperBound.block<Nc+1,1>(2*Nc,0) = ub;

    // linearMatrix.block<2*Nu*Nc,Nu*Nc+1>(0,0) = A_cons;
    // linearMatrix.block<Nc+1,Nc+1>(2*Nu*Nc,0) = Eigen::Matrix<double,Nc+1,Nc+1>::Identity();
...
int main(int argc, char * argv[])
{
    ...
    // ros::Subscriber vehicle_param_sub = nh_.subscribe<vehicle_info_msg::vehicle_info_msg>("/vehicle_info",2,vehicle_info_cb);
    // ros::Subscriber gp_com_sub = nh_.subscribe<vehicle_info_msg::gp_msg>("/vehicle_gp_com",1,vehicle_gp_com_cb);

    // ros::Publisher vehicle_control_pub = nh_.advertise<vehicle_info_msg::vehicle_control_msg>("/vehicle_control",1);
